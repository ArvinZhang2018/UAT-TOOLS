...
import com.fsc.generate.model.dto.biz.CreateRechargeCardReq;
...
public class CCApi {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CCApi.class);
    ...
    public RechargeCardResp createRechargeCard(CreateRechargeCardReq req) throws CrmException {
        try {
            Response<ResponseMsgSingle<RechargeCardResp>> response = ccApiInterface.createRechargeCard(
                    0, "CHARGE", "BIZ_BY_DAYS", req.getDays(), 0L,
                    "UAT TEST", "TEST FOR UAT", 0, req.getPlan()
            ).execute();
            if (Objects.nonNull(response)
                    && response.isSuccessful()
                    && Objects.nonNull(response.body())) {
                ResponseMsgSingle<RechargeCardResp> accountCardResponse = response.body();
                if (Objects.nonNull(accountCardResponse)
                        && accountCardResponse.isSuccess()
                        && Objects.nonNull(accountCardResponse.getResult())) {
                    return accountCardResponse.getResult();
                }
            }
        } catch (Exception e) {
            logger.error("[createAccountCardV2] [ERROR]", e);
        }
        throw CrmException.newCrmException(CrmCode.RET_API, CrmCode.ERR_API_CC_CREATE_RECHARGE_CARD_FAILURE);
    }
